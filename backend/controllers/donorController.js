...
exports.suggestSplit = async (req, res) => {
    try {
        ...
        const needs = await Need.find({ status: 'live' }).populate('ngoId', 'name trustScore impactScore');
        const campaigns = await Campaign.find({ status: 'live' }).populate('ngoId', 'name trustScore impactScore');
        ...
        const candidates = (filtered.length > 0 ? filtered : rawCandidates)
            .map(c => ({
                ...
                ngoTrustScore: c.ngoId.trustScore || 50,
                ngoImpactScore: c.ngoId.impactScore || 50,
                category: c.category,
                beneficiaries: c.beneficiaries || 0,
                deadline: c.deadline,
                funding_gap: Math.max(0, (c.amount || c.targetAmount) - (c.fundsRaised || 0))
            }))
            .filter(c => c.funding_gap > 0)
        ...
        try {
            ...
            // Fallback (Smart Heuristic Split)
            const suggestion = candidates.slice(0, 3).map((item, index, arr) => {
                const count = arr.length;
                const percentage = count === 3 ? [40, 35, 25][index] : count === 2 ? [60, 40][index] : 100;
                const amount = (donationAmount * percentage) / 100;
                return {
                    targetId: item._id,
                    ...
                    reason: item.urgency === 'high' ? "Prioritized due to critical urgency signals." : "High trust-score allocation for sustainable impact."
                };
            });
