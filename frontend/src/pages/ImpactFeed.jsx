...
function StoryCard({ story, idx }) {
    return (
        <motion.div
            ...
            className="modern-card group bg-white border-2 border-slate-100 hover:border-orange-200 overflow-hidden cursor-default transition-all duration-700 hover:shadow-3xl hover:shadow-orange-500/10"
        >
            <div className="h-80 bg-slate-100 relative overflow-hidden">
                <img
                    src={story.media?.[0] || 'https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?auto=format&fit=crop&q=80&w=1000'}
                    alt=""
                ...
                <div className="absolute top-6 left-6">
                    <div className="glass-morphism px-4 py-2 rounded-2xl border border-white/40 shadow-2xl flex items-center gap-2 text-slate-900">
                        ...
                        <span className="text-[10px] font-black text-slate-800 uppercase tracking-widest">Proof Verified</span>
                    </div>
                </div>

                <div className="absolute bottom-6 right-6">
                    <button className="w-14 h-14 bg-orange-500 text-white rounded-full flex items-center justify-center shadow-2xl shadow-orange-500/40 transform scale-0 group-hover:scale-100 transition-transform duration-500 hover:bg-orange-600">
            ...
            <div className="p-10">
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <span className="text-[10px] font-black text-orange-500 uppercase tracking-[0.2em]">{story.category || 'Impact Success'}</span>
                </div>
                ...
                <div className="pt-8 border-t border-slate-50 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-1.5 mt-0.5">
                                <p className="text-xs font-bold text-slate-700 uppercase tracking-tighter line-clamp-1">{story.ngoId?.name || 'Verified Provider'}</p>
                                <span className={`px-2 py-0.5 rounded-full text-[7px] font-black uppercase tracking-widest border ${(story.ngoId?.trustScore || 70) >= 85 ? 'bg-green-50 text-green-600 border-green-100' :
                                    (story.ngoId?.trustScore || 70) >= 70 ? 'bg-orange-50 text-orange-600 border-orange-100' :
                                        'bg-red-50 text-red-600 border-red-100'
                                    }`}>
                                    {(story.ngoId?.trustScore || 70) >= 85 ? 'High Trust' :
                                        (story.ngoId?.trustScore || 70) >= 70 ? 'Med Trust' : 'Low Trust'}
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="text-right text-slate-900">
                        <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Released</p>
                        <p className="text-lg font-black text-slate-900 tracking-tighter">₹{story.totalImpact?.toLocaleString()}</p>
                    </div>
                </div>
